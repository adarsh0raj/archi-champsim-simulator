...
void P_OPT::registerDataType(intptr_t addr, int dataTypeID, int numElements, size_t elemSz, int totalDataTypes)
{
...
void P_OPT::registerGraph(Graph &g, bool isPull)
{
...
void P_OPT::registerOffsetMatrix(uint8_t* offsetMatrix, int32_t numEpochs, int32_t numCacheLines, int vtxPerLine, int dTypeID)
{
...
void P_OPT::updateRegIndex(int32_t index, int tid)
{
    m_regIndex[tid] = index;
}

void P_OPT::updateIrregRanges(int32_t startID, int32_t endID)
{
...
void P_OPT::insertData(intptr_t addr, int tid, bool isWrite, bool &hit, bool &writeback, intptr_t &evictedAddr, bool updateReplacement)
{
    ...
    PIN_GetLock(&m_setLocks[setID], tid+1);
    {
        ...
        {
            // NOTE: Assumption that if a L2 WB misses in the P_OPT, then
            // we forward the request to DRAM and dont bring it to the P_OPT
...
uint64_t P_OPT::reduceNumMisses()
{
    uint64_t misses {0};
    for (int tid = 0; tid < numThreads; ++tid)
    {
...
void P_OPT::reportTotalStats()
{
    ...
    std::cout << "[P_OPT-STAT] Total Hits = " << totalHits << std::endl;
    ...
    std::cout << "[P_OPT-STAT] Total Misses = " << totalMisses << std::endl;
    ...
    std::cout << "[P_OPT-STAT] Total Writebacks = " << totalWritebacks << std::endl;
}

void P_OPT::reportDataTypeMisses()
{
    ...
    for (int dType = 0; dType < m_numDataTypes; ++dType)
    {
        ...
        {
            std::cout << "[P_OPT-STAT] Hits for DataType - " << names[dType] << " - " << totalHits << std::endl;
            std::cout << "[P_OPT-STAT] Misses for DataType - " << names[dType] << " - " << totalMisses << std::endl;
            std::cout << "[P_OPT-STAT] Writebacks for DataType - " << names[dType] << " - " << totalWritebacks << std::endl;
            std::cout << "[P_OPT-STAT] Deadlines for DataType - " << names[dType] << " - " << totalDeadlines << std::endl;
            std::cout << "[P_OPT-STAT] Avg. ReReference for DataType - " << names[dType] << " - " << static_cast<double>(totalReferences) / static_cast<double>(totalMisses) << std::endl;
        }
        else
        {
            assert(false);
...
void P_OPT::reportEvictionReasons()
{
    ...
    for (int d = 0; d <= numDataTypes; ++d)
    {
        ...
        std::cout << "[P_OPT-STAT] No. of Evictions by DataType - " << names[d] << " - " << evictCtr << std::endl;
    }
    #endif
    ...
    for (int dEvictee = 0; dEvictee <= m_numDataTypes; ++dEvictee)
    {
        if (dEvictee < numMainDataTypes)
            std::cout << "[P_OPT-EVICT-STAT] Eviction stats for DataType - " << names[dEvictee] << std::endl;
        else
            std::cout << "[P_OPT-EVICT-STAT] Eviction stats for DataType - others" << std::endl;
        ...
        for (int dEvicter = 0; dEvicter <= m_numDataTypes; ++dEvicter)
        {
            ...
            if (dEvicter < numMainDataTypes)
                std::cout << "[P_OPT-STAT] No. of Evictions by DataType - " << names[dEvicter] << " - " << evictCtr << std::endl;
            else
                std::cout << "[P_OPT-STAT] No. of Evictions by DataType - others - " << evictCtr << std::endl;
            
        }
        std::cout << "[P_OPT-EVICT-STAT] ~~~~~~~~~~~~~~~~~~~~~~~~\n";
    }
}


bool P_OPT::isCacheHit(intptr_t addr, int setID, bool isWrite, bool updateReplacementMetadata)
{
...
bool P_OPT::installNewLine(intptr_t addr, int setID, intptr_t &evictedAddr, bool isWrite, int setType, int tid)
{
...
int P_OPT::findSet(intptr_t addr)
{
...
int P_OPT::getReplacementIndex(intptr_t addr, int setID, int setType, int tid)
{
...
void P_OPT::updateReplacementState(int setID, int wayID)
{
    assert(m_rrpv[setID][wayID] <= m_MAX_rrpv); //checking for overflows
    m_rrpv[setID][wayID] = 0; //Hit Promotion policy
}

void P_OPT::moveToMRU(int setID, int wayID)
{
    ; // may be required for aggressive pinning of hubs
}


void P_OPT::updateHits(int tid)
{
	m_hits[tid] += 1;
}


void P_OPT::updateMisses(int tid)
{
	m_misses[tid] += 1;
}

void P_OPT::updateWritebacks(int tid)
{
    m_writebacks[tid] += 1;
}

/* For srcData accesses (find if this was a hub or non-hub) */
int P_OPT::findVtxID(intptr_t addr, int dTypeID)
{
...
uint8_t P_OPT::findRereferenceVal(int irregInd, int regInd, int tid, int dTypeID)
{
...
bool P_OPT::isSpecialDataType(intptr_t addr, int &dataTypeID)
{
    if (addr == -1)
        return false;
    assert(addr > 0);
...
bool P_OPT::winningPolicy(int bankID)
{
...
int P_OPT::determineSetType (int setID, int bankID)
{
